            ...
            if os.path.isdir(full_path):
                ...
                try:
                    ...
                    raise DollarException(
                        DollarErrorMessages.FILE_NOT_FOUND.format(full_path)
                    ) from e
                except Exception as e:
                    raise DollarException(
                        DollarErrorMessages.FAILED_TO_OPEN_FILE.format(full_path)
                    ) from e
                full_path_split = os.path.normpath(full_path).split(os.path.sep)
                open_dir_start_split = os.path.normpath(_open_dir_start).split(
                    os.path.sep
                )
                ...
                try:
                    ...
                    raise DollarException(
                        DollarErrorMessages.FAILED_TO_READ_FILE.format(full_path)
                    ) from e
