...
class DollarObjectCreationHelper:
    @staticmethod
    def init(
        path: str, content: str, dollar_id_map: Optional[DollarObjectIdMap]
    ) -> DollarObjectImpl:
        dollar_object = DollarObjectImpl(
            content, path, path[:-1], "/" + path[:-1].replace("\\", "/")
        )
        ...
        while True:
            if validate_plugin.extends() in visited:
                raise DollarExecutionException(
                    "Extension plugin {} in Dollar Object {} was caught in a circular dependency loop".format(
                        primary_plugin.get_name(), dollar_object.get_id()
                    ),
                    dollar_context,
                )
            ...
            if validation is not None:
                raise DollarExecutionException(
                    "Validation on {} failed: {}".format(
                        dollar_object.get_id(), validation
                    ),
                    dollar_context,
                )
            if validate_plugin.extends() is not None:
                ...
                try:
                    validate_plugin = plugin_map.get_extension(
                        validate_plugin.extends()
                    )
                except DollarException as e:
                    raise DollarExecutionException(
                        e.get_message(), dollar_context
                    ) from e
            else:
                break
    ...
    def transform_header_secondary(
        ...
        for key in dollar_object.get_header().keys():
            if plugin_map.has_extension_with_secondary_key(key):
                plugin_map.get_extension_from_secondary_key(key).exec_secondary(
                    dollar_object
                )  # TODO: Handle backrefs in dollar instead
