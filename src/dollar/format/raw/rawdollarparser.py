                ...
                if dollar_open in (0, 3):
                    ...
                    if (
                        ...
                        if dollar_open == 1:
                            ...
                            result.append(
                                cls._create_result_reference(
                                    dollar_text, dollar_context
                                )
                            )
                            dollar_open = 0
                            ...
                            result.append(
                                cls._create_result_function(text, dollar_context)
                            )
                            dollar_open = 0
                            text = ""
                            dollar_context_start = DollarContext(file, line, col)
    ...
    def _create_result_function(cls, text, dollar_context: DollarContext):
        ...
        if len(text_split) != 2:
            raise DollarExecutionException(
                DollarErrorMessages.DOLLAR_FUNCTION_NOT_FORMATED_CORRECTLY, dollar_context
            )
        ...
        if len(param_str_split) == 1 and param_str_split[0] == "":
            ...
            for param in param_str_split:
                ...
                dollar_context_param_start = DollarContext(
                    dollar_context.get_file(), dollar_context.get_line(), col_start_diff
                )
                ...
                if len(param_content) == 0:
                    ...
                    raise DollarExecutionException(
                        DollarErrorMessages.PARAMETER_CANNOT_BE_EMPTY.format(func_name),
